...
from vllm.v1.attention.backends.flex_attention import FlexAttentionImpl, FlexAttentionMetadata
...
def run_attention_torch(batch_size,
    ...
    if is_context_phase:
        ...
        # Each block holds 64 tokens (block_size), and we need enough blocks to store input_len tokens per sequence
        ...
        kv_cache = torch.zeros((2, total_blocks, block_size, num_key_value_heads, head_dim), 
        ...
        block_table = torch.arange(0, total_blocks, dtype=torch.int32, device=device).reshape(batch_size, num_blocks_per_seq)
        ...
        class DummyLayer(torch.nn.Module):
            def __init__(self, num_heads, num_key_value_heads, head_dim, device):
                ...
                self.register_buffer("_k_scale_base", torch.ones(num_key_value_heads, dtype=torch.float32, device=device))
                self.register_buffer("_v_scale_base", torch.ones(num_key_value_heads, dtype=torch.float32, device=device))
        ...
        for i in range(warming_up):
            #DEBUG
            #print(f"q: {q.shape}, k: {k.shape}, v: {v.shape} kv_cache: {kv_cache.shape} attn_metadata: {attn_metadata.num_actual_tokens} output: {output.shape}")
        ...
        if use_fp8_kv_cache:
            ...
            kv_cache_dtype = torch.float16
        kv_cache = torch.zeros((num_blocks, 2, block_size, num_key_value_heads, head_dim), 
                             dtype=kv_cache_dtype, device=device)
...
def get_context_attention_test_cases(if_unit_test=False):
    has_fp8_kv_cache = getSMVersion() > 86
    test_cases = []

    if if_unit_test==False:
        b_list = [1, 16, 128, 256]
        n_list = [8, 32, 64]
        s_list = [64, 1024, 8192, 65536]
        n_kv_list = [1, 4, 8]
    else:
        b_list = [1]
        s_list = [64]
            ...
            for b in sorted(b_list, reverse=True):
                for n_kv in n_kv_list:
                    ...
                    if has_fp8_kv_cache:
                        ...
                        #flashatten impl does not support fp8 context fmha
