...
def parallel_run(tasks, func, num_processes):
        ...
        while progress_value.value < len(tasks):
            ...
            for i, p in enumerate(processes):
                if p.is_alive():
                    ...
                    # Note: The exit code might be None immediately after a process fails.
                    if p.exitcode is not None:
                        print(f"Process {i} on device {i} died with exit code {p.exitcode}, not restarting.")
            
            # If no processes are alive and tasks are not yet complete, it indicates a collective failure.
            if num_alive_processes == 0 and progress_value.value < len(tasks):
                print("All worker processes have failed. Exiting the loop.")
    ...
    # Update the progress bar one last time after the loop to reflect the final state.
    progress_bar.n = progress_value.value
    progress_bar.refresh()
...
def collect_vllm(num_processes : int):
    ...
    print(f"collected gemm_vllm test cases for VLLM {version}")
    ...
    print(f"collected context attention test cases for VLLM {version}")
    
    # TODO: currently not use generation_attention
    # test_cases = vllm_v1.collect_attn.get_generation_attention_test_cases()
    # parallel_run(test_cases, vllm_v1.collect_attn.run_attention_torch, num_processes)
    # print(f"collected generation attention test cases for VLLM {version}")        
