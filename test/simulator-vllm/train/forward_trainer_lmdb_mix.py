...
def clean_data_list(all_data, target_key='time_ms', percentile=99.5):    
    ...
    valid_times = [
        d[target_key] for d in all_data 
        if target_key in d and d[target_key] is not None and d[target_key] > 0
    ]

    if not valid_times:
        print("Error: No valid time data found > 0.")
        return all_data

    upper_limit = np.percentile(valid_times, percentile)
    cleaned_data = [
        d for d in all_data
        if target_key in d and d[target_key] is not None and d[target_key] > 0.0 and d[target_key] < upper_limit
    ]

    removed_count = len(all_data) - len(cleaned_data)
    print(f"=== Data Cleaning ({target_key}) ===")
    print(f"Original samples : {len(all_data)}")
    print(f"Threshold (p{percentile}): {upper_limit:.2f}")
...
def parse_raw_features(log_file: str, model_uri: str, platform_perf: PlatformPerf, target: str, stage: str = '', disaggregation_mode: bool = False) -> List[Dict]:
    ...
    for idx, timing_data in enumerate(log_data):
        ...
        time_ms = timing_data.get('gpu_forward_time_ms', 0.0)
        if target == 'post-forward':
            time_ms = timing_data.get('total_sampling_time_ms', 0.0)
        if target == 'pre-forward':
            if idx == len(log_data) - 1:
                time_ms = timing_data['total_time_ms'] - timing_data['gpu_forward_time_ms'] - timing_data['total_sampling_time_ms']
            else:
...
def main():
    ...
    # for task in tasks:
    #     data, failed = process_single_rank_log(task)
    ...
    df.to_csv(f"{model_dir}/features.csv")
