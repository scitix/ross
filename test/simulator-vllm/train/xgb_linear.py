...
def perform_xgboost_regression(df: pd.DataFrame,
    ...
    if target == 'forward': # or target == 'pre-forward':
        ...
        raise RuntimeError(f"target={target} Unknown to trainer")
    ...
    model.fit(X.values, y, eval_set=eval_set, verbose=3000)
