...
from sgl_sim.bench_sglang import find_best_colocate_result_under_constraints, find_best_disagg_result_under_constraints
...
from common.plot import plot_and_save_distribution

def extract_metrics(sim_result = None, framework_result=None, columns=None, backend = None):
    sim_result.update({"framework": "ROSS"})
    row1 = [sim_result[col] for col in columns]

    if not framework_result:
        return pd.DataFrame([row1], columns=columns)

    framework_result.update({"framework": backend})
    row2 = [framework_result[col] for col in columns]

    return pd.DataFrame([row1, row2], columns=columns)

def run_sim_predict(model, parallel, runtime_config: RuntimeConfig, platform_perf_yaml, trace_configs = None):
...
def main():
    ...
    setup_logging('./log/ross_sgl_predict_disagg.log', args.debug)
    ...
    for input in conf.inputs:
        ...
        for backend_opts in conf.backend_opts:
            if len(conf.args['sglang']) > 0:
                conf_args = conf.args['sglang'][0]
                ...
                if "chunked_prefill_size" in conf_args:
                    ...
                    if backend_opts[0] == 'B200': # TODO: Hard code to B200 default configs
                        chunked_prefill_size_list = [16384]
                                ...
                                for mem_fraction_static in mem_fraction_static_list:
                                    ...
                                    if not args.no_comparison:
                                        ...
                                        if not os.path.exists(conf.test_dst):
                                            ...
                                            continue
                                        trace_config_dict = resolve_sglang_log_paths(
                                    ...
                                    if not os.path.exists("arrivals/sglang"):
                                        os.makedirs("arrivals/sglang")
                                    arrival_path = (
                                        f"arrivals/sglang/arrivals_{model_name}_{datatype}_batch_{batch}_promptnum_{conf.num_prompt}_rate_{rate}.jsonl"
                                    )
                                    if not os.path.exists(arrival_path):
                                        call_bench_serving(model, 'sglang', dataset, data_path, \
                                            str(isl), str(osl), str(rate), str(conf.num_prompt), arrival_path, str(batch),
                                        )
                                    ...
                                    if not args.no_comparison: # 1. EVAL: Load Trace Logs
                                        if parallel.find("@") == -1: # COLOCATE
                                            trace_result_dict = load_traces_lmdb.load_traces(
                                                trace_config_dict, args.debug, skip_timing
                                            )
                                        else:
                                            trace_result_dict = load_disaggregation_traces_lmdb.load_traces(
                                                trace_config_dict, args.debug, skip_timing
                                            )
                                    ...
                                    sim_result_dict = run_sim_predict(model, parallel, runtime_config, hw_yaml, {})
