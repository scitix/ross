...
def _create_bench_client_session():
    ...
    BENCH_AIOHTTP_TIMEOUT_SECONDS = 6 * 60 * 60  # 6h
...
async def async_request_sglang_generate(
    ...
    async with _create_bench_client_session() as session:
        ...
        # print("[BENCH OUT]", api_url, headers, payload)
        output = RequestFuncOutput.init_new(request_func_input)
            ...
            async with session.post(
                ...
                if response.status == 200:
                    async for chunk_bytes in response.content:
                        ...
                        latency = time.perf_counter() - st
                        if chunk == "[DONE]":
                            # print("Request Ending Done:", time.strftime("%H:%M:%S"))
                            pass
                        else:
                            data = json.loads(chunk)
                            ...
                            if "text" in data and data["text"]:
                                ...
                                if ttft == 0.0:
                                    ttft = time.perf_counter() - st
                                    output.ttft = ttft
                                    # print(f"ttft debug: {ttft + st} - {st} = {ttft}")
                                ...
                                # Decoding phase
                                    ...
                                    # if output_len >= 2498:
                                    #     print("Request Ending:", time.strftime("%H:%M:%S"))
...
def get_dataset(args, tokenizer):
    ...
    if args.dataset_name == "sharegpt":
        ...
        input_requests = sample_sharegpt_requests(
            ...
            tokenizer=tokenizer,
            max_prompt_len=args.sharegpt_prompt_len,
            max_output_len=args.sharegpt_output_len,
...
def maybe_oversample_requests(
    ...
    """
    Oversample the list of requests if its size is less than the desired number.
    """
    if no_oversample:
        print(f"Skipping oversampling. Total samples: {len(requests)}.")
        return

    if len(requests) == 0:
        print("No requests to oversample.")
        return

    if len(requests) < num_requests:
        needed = num_requests - len(requests)
        additional = [deepcopy(requests[i % len(requests)]) for i in range(needed)]
        requests.extend(additional)
        print(f"Oversampled requests to reach {len(requests)} total samples.")
...
def sample_sharegpt_requests(
    ...
    tokenizer: PreTrainedTokenizerBase,
    fixed_output_len: Optional[int] = None,
    max_prompt_len: Optional[int] = 0, # prompt_len <= max_prompt_len
    max_output_len: Optional[int] = 0, # output_len <= max_output_len
    ...
    for i in range(len(dataset)):
        ...
        if prompt_len < 4 or (fixed_output_len is None and output_len < 4):
            ...
            continue
        if (max_prompt_len > 0 and prompt_len > max_prompt_len) or (max_output_len > 0 and output_len > max_output_len):
            # Prune too long sequences.
            continue
        filtered_dataset.append(
            DatasetRow(prompt=prompt, prompt_len=prompt_len, output_len=output_len if max_output_len == 0 else max_output_len)
        )
...
async def benchmark(
    ...
    async def limited_request_func(request_func_input, pbar):
        if semaphore is None:
            ...
            output.end_time_perf = time.perf_counter()
            return output
    ...
    # print("warmup_output:", [(w.prompt_len, w.output_len) for w in warmup_outputs])
    ...
    print("Benchmark Start:", time.strftime("%H:%M:%S"))
    ...
    last_end_perf = max((o.end_time_perf for o in outputs), default=0.0)
    if last_end_perf > 0:
        # last_end_wall = benchmark_start_time + (last_end_perf - benchmark_start_time)
        print("Last Request Finished:", time.strftime("%H:%M:%S"))
    ...
    print("Benchmark Ends:", time.strftime("%H:%M:%S"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-oversample",
        action="store_true",
        help="If set, do not oversample when the dataset has fewer prompts than --num-prompts.",
    )
    parser.add_argument(
        "--sharegpt-output-len",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The prompt length of the model for the ShareGPT dataset. Requests longer than the prompt length will be dropped.",
    )
    parser.add_argument(
        "--random-input-len",
        type=int,
